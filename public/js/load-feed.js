...
fetch(`${window.location.origin}/api/posts/user/${window.location.pathname.replace('/profile/', '')}`).then(async json => {
	...
	if(typeof posts.code != 'undefined') {
		...
		if(posts.code == 10010) {
			...
			errMessage.innerHTML = `<i class='fas fa-bomb'></i><br> Couldn't find the user profile!`
		}
		if(posts.code == 10011) {
			errMessage = document.createElement('p')
			errMessage.classList.add('text-xl')
	...
	postIO.on('posted', async post => {
		if(posts[0]._author == post._author) {
