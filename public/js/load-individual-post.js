...
fetch(`${window.location.origin}/api/post/${window.location.pathname.replace('/post/', '')}`).then(async json => {
	const post = await json.json()
	if(typeof post.code != 'undefined') {
		let errMessage
	...
	// Wrapper
	postWrapper.classList.add('post-wrapper-full')
	postWrapper.classList.add('flex')
	postWrapper.classList.add('flex-col')
	// postWrapper.classList.add('justify-between')
	...
	postHeader.appendChild(postBoard)
